...
(function () {
    ...
    function curatorUrl(name) {
        const urls = window.CURATOR_URLS || {};
        const value = urls[name];
        if (!value) throw new Error('Missing Playlist Curator URL: ' + name);
        return value;
    }

    function curatorStreamUrl(itemId) {
        return curatorUrl('streamTemplate').replace('__ITEM_ID__', encodeURIComponent(itemId));
    }

    window.curatorUrl = curatorUrl;
    window.curatorStreamUrl = curatorStreamUrl;
    ...
    async function submitPlaylistSave(payload, successMessage, submittedRevision) {
        ...
        renderWorkbench();
        try {
            const res = await fetch(curatorUrl('savePlaylist'), {
                method: 'POST',
    ...
    async function findDuplicatesForTracks(tracks, mode, options) {
        ...
        try {
            const res = await fetch(curatorUrl('findDuplicates'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
